...
function MovieInformation() {
  ...
  const addToFavorites = async () => {
    await axios.post(`https://api.themoviedb.org/3/account/${user.id}/favorite?api_key=${process.env.REACT_APP_TMDB_KEY_II}&session_id=${localStorage.getItem('session_id')}`, {
      media_type: 'movie',
      media_id: id,
  ...
  const addToWatchlist = async () => {
    await axios.post(`https://api.themoviedb.org/3/account/${user.id}/watchlist?api_key=${process.env.REACT_APP_TMDB_KEY_II}&session_id=${localStorage.getItem('session_id')}`, {
      media_type: 'movie',
      media_id: id,
  ...
  if (error) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center">
        <Link to="/">Something went wrong, Click to go back to Homepage</Link>
      </Box>
    );
  }
  ...
  return (
    <Grid container className={classes.containerSpaceAround}>
      <Grid display="flex" justifyContent="center" marginBottom="30px" item sm={12} lg={4}>
          ...
          <div className={classes.buttonsContainer}>
            ...
            <Grid item xs={12} sm={6} className={classes.buttonsContainer}>
              <ButtonGroup size="medium" variant="outlined">
                ...
                <Button endIcon={<ArrowBack />} sx={{ borderColor: 'pramary.main' }}>
      ...
      <Box marginTop="5rem" width="100%">
        ...
        {recommendations && recommendations.total_results ? <MovieList numberOfMovies={12} movies={recommendations} /> : <Box><Typography variant="h4" mt={5} align="center">Sorry, nothing was found.</Typography></Box>}
